# -*- coding: utf-8 -*-
"""
Created on Fri Jul  8 11:36:01 2022
...
if Ice_variable=='Y':
    ...
    iceoff_name='days_from_iceoff'# input('name of iceoff variable:')
...
zoo_variable='Y'#input('Zooplankton variable? (Y/N)')
if zoo_variable=='Y':
    zoo_name='Daphnia'
    modify='Y'
    if modify=='Y':
...
test_df=all_df.copy()
binary_f='Inva_spe'#input('Name of binary feature: ')
...
zoo_c='Y'#input('Zooplankton categorization? (Y/N)')
if zoo_c=='Y':
...
## Test time
training_yr_df={}
test_time = [pd.Timestamp(2010,1,1),pd.Timestamp(2015,12,31)]
test_yr=list(range(test_time[0].year,test_time[1].year+1))

Nut_num =2 #int(input('How many nutrients to predict? (include Chl and put them in order) '))
Nut=['DO_sur', 'DO_bot']
Nut_f = ['Date']+Nut
Nut_df=all_df[Nut_f].dropna()

Lake_Nut_metrics = pd.DataFrame(columns = Nut_f[1:],index=['MAE','RMSE','R2'])
# set the hyperparameters
hyperparameters = {'n_batch':20,'nb_epoch':100,'n_neuros':50,'dropout':0.2,'time_steps':7,
...
for i in range(30):
    ...
    if id(training_yr) not in ids:
        ...
        features = ['Date', 'accu_P','delT', 'U', 'AirT', 'Prec', 'SWR', 'inflow', 'outflow', 
                    ...
                    'NIT_nit', 'OGM_docr', 'Sum_P_lastyr', 'Inva_spe','days_from_iceoff_-3',
                    'days_from_iceoff_-2', 'days_from_iceoff_-1', 'days_from_iceoff_0',
                    'days_from_iceoff_1', 'days_from_iceoff_2','days_from_iceoff_3',
                    'Ice_d_long', 'Ice_d_middle', 'Daphnia_high', 'Daphnia_little',
                    'Daphnia_low', 'Daphnia_middle']
