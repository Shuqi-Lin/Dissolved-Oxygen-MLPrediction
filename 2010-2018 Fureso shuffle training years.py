# -*- coding: utf-8 -*-
"""
Created on Wed Jul  6 18:11:17 2022
...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul  5 11:41:24 2022
...
all_df = pd.read_csv('Fureso_Observation_df_nowinter_classification.csv',sep = '\t',parse_dates=['Date'])

#%% Preprocess training features
print(all_df.columns)
Ice_variable='N' #input('Ice variables? (Y/N)')
if Ice_variable=='Y':
    ## Modifiy iceoff_day variable
    iceoff_name='days from iceoff date'# input('name of iceoff variable:')
...
test_df=all_df.copy()
binary_f='Treatment'#input('Name of binary feature: ')
test_df[binary_f]=pd.Categorical(test_df[binary_f])
Ice_c='N'#input('Ice categorization? (Y/N)')
if Ice_c=='Y':
...
## Test time
training_yr_df={}
test_time = [pd.Timestamp(2010,1,1),pd.Timestamp(2018,12,31)]
...
for i in range(20,30):
    ...
    if id(training_yr) not in ids:
        ...
        features = ['Date','inflow', 'thermD', 'delT', 'U', 'AirP', 'AirT', 'Hum','CC', 'MLD', 'W', 'Treatment']
